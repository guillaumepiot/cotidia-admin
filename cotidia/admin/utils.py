...
FIELD_MAPPING = {
    "DateTimeField": (lambda: {"display": "datetime", "filter": "date"}),
    "UUIDField": (lambda: {"display": "verbatim", "filter": "text"}),
    "DateField": (lambda: {"display": "date", "filter": "date"}),
    "TimeField": (lambda: {"display": "date", "filter": "date"}),
    "CharField": (lambda: {"display": "verbatim", "filter": "text"}),
    "ChoiceField": (lambda: {"display": "verbatim", "filter": "choice"}),
    "IntegerField": (lambda: {"display": "verbatim", "filter": "number"}),
    "DecimalField": (lambda: {"display": "verbatim", "filter": "number"}),
    "BooleanField": (lambda: {"display": "boolean", "filter": "boolean"}),
...
def get_field_representation(
    field_name, field, prefix="", max_depth=MAX_SUBSERIALIZER_DEPTH
):
    ...
    sorted_types = sorted(SUPPORTED_FIELD_TYPES_SERIALIZER, key=lambda x: len(x.mro()))

    field_type = next(iter([t for t in sorted_types if isinstance(field, t)]), None)
    ...
    if issubclass(field_type, serializers.ModelSerializer):
        if max_depth <= 0:
            ...
            return get_fields_from_serializer(
                field, prefix=("%s%s__") % (prefix, field_name), max_depth=max_depth - 1
            )
    ...
    if hasattr(field, "choices"):
        field_representation["options"] = list(
            map(lambda x: {"value": x[0], "label": x[1]}, field.choices.items())
        )
...
def get_fields_from_serializer(
    serializer, prefix="", max_depth=MAX_SUBSERIALIZER_DEPTH
):
    fields_representation = {}
    fields = serializer.fields
    for f in fields:
        field_representation = get_field_representation(f, fields[f], prefix, max_depth)
...
def get_serializer_default_columns(serializer):
    ...
    try:
        ...
        fields = [
            f
            for f in get_fields_from_serializer(serializer).keys()
            if "__" not in f
            if "uuid" not in f
        ]
    return fields
...
def get_object_options(app_label, model_name, query):
    ...
    results = []
    for item in model.objects.filter(q_objects):
        results.append({"label": item.__str__(), "value": item.id})
...
def get_queryset(model_class, serializer_class, filter_args=None):
    ...
    for field in field_repr.keys():
        ...
        if filter_params:
            ...
            if hasattr(serializer, "filter_" + field):
                ...
                qs = FILTERS[filter_type](qs, field + suffix, filter_params)
    ...
    if ordering_params:
        ...
        clean_field_name = first_field[1:] if first_field[0] == "-" else first_field
        try:
            condition_blank = Q(**{clean_field_name + "__exact": ""})
            annotation = {
                "val_is_empty": Count(
                    Case(When(condition_blank, then=1), output_field=CharField())
                )
            }
